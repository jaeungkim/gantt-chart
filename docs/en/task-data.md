...
The five `allowX` flags and `readOnly` resolve most-specific-first, `task.allowX` before `task.readOnly` before the chart's config; see [Editing tasks](editing.md). `minDate` and `maxDate` do not use that chain — the task's value replaces the chart's when present. `color` and `className` and the CSS variables a colored bar sets are covered in [Custom rendering](custom-rendering.md). `lane` is covered in [Grouping and swimlanes](grouping.md).
...
| Field | What it needs |
|---|---|
| `parentId` | `hierarchy` — without it no tree is built, depth comes from `sequence`, and nothing is a summary row. See [Task list and hierarchy](task-list.md). |
| `manuallyScheduled` | `schedulingPolicy` set to something other than `'off'`. See [Scheduling](scheduling.md). |
| `TaskDependency.lag` | `schedulingPolicy`, or `criticalPath`. A `lag: 5` link draws exactly like `lag: 0` on a chart with neither. |
| `lane` | nothing. Packing runs with or without `groupBy`; with `groupBy` on, a lane packs inside its own group only. See [Grouping and swimlanes](grouping.md). |
| `baselineStart` / `baselineEnd` | **nothing.** A baseline renders as soon as `baselineStart` is present, and it widens the timeline range. There is no prop gate and no way to switch it off other than removing the field. See [Scheduling](scheduling.md). |
...
What the four types mean for scheduling, and how arrows are drawn and deleted, is in [Dependencies](dependencies.md).
...
The library rewrites `sequence` in exactly one situation. A row reorder renumbers **every** task depth-first from the resulting tree, and writes `parentId` on the moved task only. Persist the whole array after a reorder, not only the row that moved. See [Reordering rows](reordering.md).
...
A parent's own `progress` always wins over the value rolled up from its children; the roll-up formula is in [Task list and hierarchy](task-list.md).
...
- **Duplicate ids are never detected.** Every array entry still gets a row, but every lookup keyed by id keeps the last occurrence. Dependencies, roll-up and undo patches resolve to the wrong task. Deduplicate before you pass the array in.
...
- **The chart never adds or removes tasks.** Drawing a new task proposes a draft through `onTaskCreate`, and the host supplies `id`, `parentId` and `sequence`. A change in row count is not invertible by a field patch, so it clears the undo history instead of adding a step.
...
- **`TaskTransformed` is output only.** The chart hands it to your renderers and column functions; `tasks` only ever accepts `Task`. Its fields are listed in [Task](ref/task.md).
- **Nothing reconciles `sequence` with `parentId`.** A child whose `sequence` sorts above its parent stays drawn above it, until the first row reorder renumbers the whole array.
...
Next: [Task list and hierarchy](task-list.md) — turning `parentId` into summary rows, and choosing the columns beside them.
