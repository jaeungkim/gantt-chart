...
Behaviour lives in the guides: [Task data](../task-data.md) for dates and `sequence`,
[Editing tasks](../editing.md) for the interaction flags, [Scheduling](../scheduling.md) for
...
The four link types and how arrows are drawn and deleted: [Dependencies](../dependencies.md).
...
[`GanttInteractionConfig`](interaction-config.md) for the full resolution order.
...
absent even with `criticalPath` on. Slack and criticality: [Scheduling](../scheduling.md) and
[`computeCriticalPath`](core-critical-path.md).

## Notes

`isMilestoneTask` and `normalizeProgress` are **not exported from the package root**. They exist in
the source and are re-exported by `src/types/task.ts`, but `src/index.tsx` does not list them, so
...
Related pages: [`GanttProps`](props.md), [`GanttInteractionConfig`](interaction-config.md),
