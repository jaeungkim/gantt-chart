...
What markers and bands are used for, and how they behave as the timeline scrolls, is in [The timeline](../timeline.md).
...
Each value is passed straight to the chart's UTC `dayjs`, so a string without a zone (`'2025-06-01'`, `'2025-06-01T09:00'`) is read as a UTC wall clock and lands where it reads. The same rules as task dates apply; see [Task data](../task-data.md).
...
- The rendered range is the tasks' span plus a fixed buffer unless `visibleStart`, `visibleEnd` or `infiniteScroll` widens it. A marker far past the last task is not drawn; see [The timeline](../timeline.md).
...
- [The timeline](../timeline.md) — when markers and bands are drawn, and what the rendered range covers.
- [Theming](../theming.md) — `--gantt-marker`, `--gantt-marker-warning`, `--gantt-marker-label`, `--gantt-today-marker`, `--gantt-band-bg` and the rest of the custom-property table.
- [Props](props.md) — the full `GanttProps` index.
