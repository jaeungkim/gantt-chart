...
Behaviour and worked examples live in [Custom rendering](../custom-rendering.md).
...
`TaskTransformed` is documented in [Task](task.md), `GanttScaleKey` in [Scales](scales.md).
...
Baselines themselves are covered in [Scheduling](../scheduling.md). Every prop named here is
indexed in [Props](props.md).
