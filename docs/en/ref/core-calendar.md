...
working-day calendar on changes in the chart is in [Scheduling](../scheduling.md).
...
A single shared instance, not a factory. It is the default for the `calendar` option of
[`scheduleTasks`](core-scheduling.md) and [`computeCriticalPath`](core-critical-path.md), so
...
Pass the calendar on as `options.calendar` to [`scheduleTasks`](core-scheduling.md) or
...
what a calendar does and does not affect is in [Scheduling](../scheduling.md).
