...
[Task list and hierarchy](../task-list.md) for when the pane appears and how the splitter behaves.
...
`render` receives a [`TaskTransformed`](task.md), not the raw `Task`, so the derived fields
...
- `columns` does nothing without a visible pane, and an explicit `showTaskList={false}` beats a
  supplied `columns` array. See [`GanttProps`](props.md).
