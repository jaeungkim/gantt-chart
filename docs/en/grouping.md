...
prop is on — see [Task](ref/task.md).
...
[Task data](task-data.md). There is no prop that sorts groups, and no prop that declares a group
...
[Task list and hierarchy](task-list.md). One flat list of strings holds both kinds of id. Task ids
...
— see [Keyboard and screen readers](accessibility.md).
...
The colors come from the `--gantt-*` custom properties in [Theming](theming.md).
...
`rowTaskId: null` — see [Editing tasks](editing.md).
...
> `allowRowReorder` are effectively incompatible — see [Reordering rows](reordering.md).
...
- **No exported helpers.** The row builder, the lane packer, and the `group:` prefix are internal.
  ...
  [Grouping types](ref/grouping.md).
...
Next: [The timeline](timeline.md) covers scales, the visible range, and zooming.
