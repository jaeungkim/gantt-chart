...
The five `allowX` flags and `readOnly` resolve most-specific-first, `task.allowX` before `task.readOnly` before the chart's config; see [Editing tasks](editing). `minDate` and `maxDate` do not use that chain — the task's value replaces the chart's when present. `color` and `className` and the four CSS variables a colored bar sets are covered in [Styling](styling). `lane` is covered in [Grouping and swimlanes](grouping).
...
- **`TaskTransformed` is output only.** The chart hands it to your renderers; `tasks` only ever accepts `Task`. Its fields are listed in [Task](ref/task).
