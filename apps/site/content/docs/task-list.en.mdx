...
outline path the chart already orders rows by, so the second child of task 2 reads `2.1`. Swimlane
bands carry no sequence and stay blank, so their labels still line up with the names below them.
...
`onCellChange` — and there is no renderer for a grid row or a group header. `renderDetail` is the
