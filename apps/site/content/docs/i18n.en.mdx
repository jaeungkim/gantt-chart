...
The header drag readout is the one place that layer prints a span rather than a point. With a `locale` set it merges what the two ends share, so a three-day move reads `Jan 15 – 17`; otherwise it joins two compact built-in labels with an en dash. Your own `tooltip` function formats both ends and is joined the same way. Two ends that format identically print one label, not the same label twice.
