...
The header drag readout formats one end of the dragged span at a time and writes it on the tick row at that end. A move writes both ends, a resize only the end it is moving. So the slot is called twice per frame with a point, not once with a span.

The exception is a span whose two ends format identically - a task shorter than the scale resolves. That prints one label for both, and with a `locale` set it is `Intl.DateTimeFormat.formatRange` that merges them, dropping what the ends share so a same-day move reads `Jan 7` rather than `Jan 7 – Jan 7`. Without a locale the two labels are joined with an en dash, and your own `tooltip` function is joined the same way.
