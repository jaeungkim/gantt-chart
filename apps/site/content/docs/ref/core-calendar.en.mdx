...
This is the same non-working-day definition the chart shades with, one level flatter: the chart's
`holidays` prop also accepts objects and date ranges, and it expands them to the plain
...
There is exactly one notion of "a day" in the engine: whatever this object says. Turning the
working-day calendar on swaps the object, and nothing else changes. `skipsNonWorkingDays` is there
so a caller can take the cheap path instead of asking the calendar day by day.
...
`workingWeekdays` and `holidays`, so a calendar you construct is for your own date
...
`snapForward` returns its input unchanged after 366 attempts. An empty `workingWeekdays` produces
...
The chart reads a calendar in one place, the bar drag, where `snapForward` moves the dragged edge
off a non-working day — see [Working calendar](../working-calendar).
