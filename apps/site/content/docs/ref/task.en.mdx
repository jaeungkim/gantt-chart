...
[Editing tasks](../editing) for the interaction flags, [Working calendar](../working-calendar)
for the baselines.
...
| `baselineEnd` | `string` | no | Planned end, UTC ISO. Without `baselineStart` it is ignored; with `baselineStart` and no `baselineEnd` the baseline is a short stub at the minimum bar width. | always |
