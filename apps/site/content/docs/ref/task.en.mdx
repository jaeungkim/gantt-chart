...
export interface Task {
  ...
  /**
   * Allows/blocks dragging this row to a new position or a new parent
   *
   * Unlike the other flags this one is off unless something turns it on: reordering is
   * opt-in, so `allowReorder` on the chart (or here) is what enables it.
   */
