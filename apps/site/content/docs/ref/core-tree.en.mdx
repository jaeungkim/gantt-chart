...
[Reordering rows](../reordering); for running these outside React, see
...
as they are. See [Task and task types](task).
...
unchanged. A fresh array is returned; the input is not mutated and the elements keep their identity.
The sort is stable, so two tasks with the same `sequence` keep their input order. How the strings
themselves compare is described in [Task data](../task-data).
...
to `onTaskMove` — see [Reordering rows](../reordering).
...
The checks run in the order the `GanttMoveRejection` table above lists them, and the first one that
fails is what comes back. It is cheap enough to call on every frame of a drag, which is how the
chart shows a blocked drop while the pointer is still down.
...
`sequence` is a path — its length is the depth, its last segment the slot among siblings — so there
is no key to insert between `"1.1"` and `"1.2"`. It renumbers instead: the forest is rearranged,
walked depth first in row order, and every task gets the path it renders at. `parentId` is rewritten on the moved task alone — its
