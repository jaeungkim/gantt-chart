...
zone name. The readout formats one end of the dragged range at a time and writes it in the tick
cell that end lands in: a move writes both ends, a resize only the end it is moving. Two ends that
land in the same cell share one label, joined by an en dash where the strings differ. The shorter
strings drop the year, which the header row above the readout is already showing; `quarter` and
`year` keep it, because a span at those scales can cross a year boundary. `day` drops the date as
well: at that scale the header row is the date and the ticks are hours, so the readout is the clock.
