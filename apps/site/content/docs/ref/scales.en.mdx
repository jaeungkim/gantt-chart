...
`GanttTooltipRenderProps.scale` and `GanttHeaderCellRenderProps.scale` both carry it. See
[Renderers](renderers).
...
- `GanttScaleFormat` functions must return a `string`. There is no `ReactNode` escape hatch here —
  to render markup in a header cell, use `renderHeaderCell` ([Renderers](renderers)).
