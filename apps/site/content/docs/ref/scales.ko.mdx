...
앱에서 직접 그리고 ref의 `setScale`로 배율을 바꾸세요. 휠과 키보드를 포함한 모든 변경은 `onScaleChange`로
...
단계의 픽셀 수를 정해요. 배율마다 무엇을 보여 주는지는 [타임라인](../timeline)에 있고 이 페이지는
한 행의 타입만 다뤄요.
...
눈금 너비는 `basePxPerDragStep / dragStepAmount`로 나와요. 그렇게 나온 하루당 픽셀 수는
[타임라인](../timeline)에 있어요. 같은 두 필드가 정하는 드래그 그리드는 [작업 편집](../editing)에 있고요.
