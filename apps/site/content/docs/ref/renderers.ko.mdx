...
세 개의 prop이 차트가 직접 그리는 마크업의 일부를 대체해요. `renderTooltip`,
`renderHeaderCell`, `renderDetail`이에요. 타입은 모두 `@jaeungkim/gantt-chart`에서 가져와요.
...
- 제스처 노드는 `gantt-sr-only`로 화면에서는 숨겨져요. 이동이나 크기 조절의 눈에 보이는 readout은
  눈금 행이 움직이는 끝에 적는 날짜예요. 막대가 같은 날짜를 한 번 더 찍지 않도록요. 노드는 그대로 마운트되고
