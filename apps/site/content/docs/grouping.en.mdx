...
function byMonth(task: TaskTransformed): string | null {
  if (task.isSummary) return null; // summary rows land in Ungrouped
...
sees `depth`, `order`, `isSummary` and bar geometry — see [Task](ref/task).
...
In the task list, a lane row shows the name of its **earliest-starting** task — `tasks[0]`
after packing, not the first one in the input array — plus a `+N` badge where N is the number of
other tasks on the row.
...
Drawing a new task is unaffected by lanes and by grouping: it only happens below the last row, so
neither a lane row nor a group header row is a target — see [Editing tasks](editing).
...
- **One task per lane row in the task list.** The name cell and the keyboard cells of a lane row act
