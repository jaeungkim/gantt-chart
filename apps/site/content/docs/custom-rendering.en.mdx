...
description: "Per-task colors and the three render props"
---
...
takes a color per task, a class name per task, and three render props that replace a node outright.
...
- **Anything that is not a background.** The bar's text color, its radius, its shadow, the
  selection outline and the dependency arrows all come from theme tokens. `color` changes the six
...
## The three render props

Past the two cheap options there are three escape hatches. Each replaces one node completely.

| Prop | Replaces | Spread onto your root |
|---|---|---|
| `renderDetail` | the detail panel body docked beside the timeline | nothing to spread |
...
The complete type shapes are in [Renderers](ref/renderers). `renderDetail` is the only one with a
page of its own: what opens the panel, its props bag and how to control it are in
...
- **There are three render props, and no more.** There is no renderer for a task-list row, a group
  header, the drag readout, the drop indicator or the dependency arrows — those are
  reachable through CSS only. The chart renders no chrome of its own to replace: no toolbar, no
  scale picker, nothing outside the timeline and the task list pane. The task list is not an
  exception any more: its one cell renders `task.name` and takes no renderer of its own — put
  per-task detail in the [detail panel](detail-panel) instead.
...
is the only option that takes capabilities away.
