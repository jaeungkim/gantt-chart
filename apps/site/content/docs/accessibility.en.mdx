...
| `Enter` / `Space` | any | An expandable row toggles. Any other row announces its task's label, without the progress, and opens the detail panel on that task when the panel is on and `detailTrigger` is not `'none'`. |
...
Nothing inside the chart flips the pane any more, so the four changes below happen when you change
...
| a reorder key, refused | nothing - the key is not claimed at all |
...
the row. A group header row has exactly one cell.
