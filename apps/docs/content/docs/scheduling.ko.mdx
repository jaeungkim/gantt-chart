---
title: "일정 계산"
description: "자동 일정, 근무일 달력, 임계 경로, 베이스라인"
---
...
않아요. 이 필드 자체는 나머지 작업 형태와 함께 [작업 데이터](task-data)에 정리돼 있어요.
...
[작업 그래프 헬퍼](ref/core-graph)에 있어요.
...
크기 조절은 아예 스냅하지 않아요. 이 경계는 [작업 편집](editing)에서 설명해요.
...
`--gantt-critical` 계열 변수에서 가져와요. 이 변수들은 나머지와 함께 [테마](theming)에 정리돼
...
같은 값을 돌려줘요. 결과 타입은 [임계 경로](ref/core-critical-path)에 있어요.
...
`columns` prop은 [작업 목록과 계층](task-list)에서 다뤄요.
...
마일스톤 규칙은 [작업 데이터](task-data)가 갖고 있어요.
...
[`scheduleTasks`](ref/core-scheduling)를 보세요.
...
[`hierarchy`](task-list)를 켜면 요약 행의 여유와 기간이 막대가 그려지는 구간과 어긋날 수 있어요.
...
다음: [행 재정렬](reordering) — 행을 옮기면 날짜가 아니라 작업 목록이 바뀌어요.
