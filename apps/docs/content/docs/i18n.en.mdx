...
Only the dates inside those ARIA sentences go through the localized `tooltip` formatter. The two strings a host can replace are `columns[].header` ([Task list and hierarchy](task-list)) and `ungroupedLabel` ([Grouping and swimlanes](grouping)), both of which default to English.
...
- **The label logic is not exported.** A host cannot reuse the chart's resolved formatters for its own toolbar or legend, and has to build its own `Intl.DateTimeFormat` for that.
