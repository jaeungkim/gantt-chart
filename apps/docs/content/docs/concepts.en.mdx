...
| task list | The left pane, the grid of columns beside the timeline. See [Task list and hierarchy](task-list). |
...
| milestone | A task with `type: 'milestone'`, drawn as a diamond at `startDate`. See [Task data](task-data). |
...
stretches the chart with nothing drawn out there. [Task data](task-data) lists every place the
field still counts.
...
that stays put. See [Scheduling](scheduling).
...
weekend on screen — the days only stop counting. [Scheduling](scheduling) says exactly what it
...
every gap exactly — see [Scheduling](scheduling).
...
A task at 100% progress is never on it — [Scheduling](scheduling) has the rule and the
...
row reorder — see [Reordering rows](reordering).
