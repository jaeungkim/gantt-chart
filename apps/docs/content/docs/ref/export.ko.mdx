...
`GanttExportApi`는 [`GanttHandle`](handle)이 확장하는 세 인터페이스 중 하나예요. 그래서 이 메서드는
`ref`로 호출해요. 호출 방식은 [명령형 API](../imperative-api)를 참고하세요.
...
기본 배경색은 컨테이너의 `--gantt-background`에서 와요. 이 변수는 [테마](../theming)에서 다뤄요.
