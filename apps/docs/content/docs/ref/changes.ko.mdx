...
prop 시그니처는 [GanttProps](props)에 모두 정리돼 있어요. 거부가 실제로 어떻게 동작하는지는 [이벤트와 취소 가능한 변경](../events)에서 다뤄요.
...
`Task`는 [Task와 작업 타입](task)에 정의돼 있어요.

`changedTasks`에 항목이 여러 개 담기는 경우는 두 가지예요. 첫째는 요약 막대를 드래그해서 하위 트리 전체가 움직인 경우예요. 둘째는 자동 스케줄링이 켜져 있고, 드롭이 후행 작업(successor)을 밀어낸 경우예요. 둘 다 [작업 편집](../editing)과 [스케줄링](../scheduling)에서 설명해요.
...
번호를 다시 매기는 규칙은 [행 순서 변경](../reordering)에 있어요.
...
`DependencyType`은 [Task와 작업 타입](task)에 정의돼 있어요. 네 가지 연결 타입은 [의존성](../dependencies)에서 설명해요.
