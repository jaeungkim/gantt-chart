...
| `calendar` | [`WorkingCalendar`](core-calendar) | `CALENDAR_DAYS` | 모든 이동과 지연(lag), 기간을 세는 날짜 단위예요. `CALENDAR_DAYS`는 주 7일을 모두 셈에 넣어요 |
...
[`TaskGraph`](core-graph)와 `linkKey`를 거쳐 전달돼요.
...
| 필드 | 타입 | 값 |
|---|---|---|
| `predecessorId` | `string` | 이 링크가 나온 [`TaskDependency`](task)의 `targetId`예요 |
| `successorId` | `string` | 그 의존성을 들고 있는 작업의 id예요 |
| `type` | [`DependencyType`](task) | `'FS'`, `'SS'`, `'FF'`, `'SF'` 중 하나예요. 각각이 무엇을 기준으로 잡는지는 [의존성](../dependencies)에 있어요 |
...
[`computeCriticalPath`](core-critical-path)를 참고하세요. `scheduleTasks`는 그중 어느 것도
보고하지 않아요.
