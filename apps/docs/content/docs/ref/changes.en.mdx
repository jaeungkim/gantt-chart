---
title: "Change and draft types"
description: "`GanttTaskChange`, `GanttReorderChange`, `GanttDependencyChange`, `GanttTaskDraft`"
---
...
Every prop signature is listed in [GanttProps](props). The behaviour behind the veto is in [Events and cancellable changes](../events).
...
`Task` is defined in [Task and task types](task).

`changedTasks` holds more than one entry in two cases: a summary bar was dragged, so its whole subtree moved; or auto-scheduling is on and the drop pushed successors. Both are described in [Editing tasks](../editing) and [Scheduling](../scheduling).
...
The renumbering rules are in [Reordering rows](../reordering).
...
`DependencyType` is defined in [Task and task types](task). The four link types are explained in [Dependencies](../dependencies).
...
Drawing is described in [Editing tasks](../editing).
