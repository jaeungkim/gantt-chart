---
title: "PNG export"
description: "`GanttExportApi`, `GanttExportOptions`, `GanttExportRange`"
---
...
`GanttExportApi` is one of the three interfaces [`GanttHandle`](handle) extends, so the method is
reached through the `ref`. See [Imperative API](../imperative-api) for the calling pattern.
...
[Theming](../theming) for that variable.
