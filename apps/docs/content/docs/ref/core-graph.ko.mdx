...
[스케줄링 코어](core-scheduling)에 있어요. 이 함수들이 읽는 `Task` 모양은
[작업](task)에 있고요. 동작은 [의존성](../dependencies)과
[스케줄링](../scheduling)에서 다뤄요. 차트 없이 코어만 돌리는 이야기는
[헤드리스 코어](../headless-core)예요.
...
[임계 경로 코어](core-critical-path)를 참고해요.
