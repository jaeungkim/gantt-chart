...
자체 멤버는 하나도 선언하지 않아요. `GanttExportApi`는 [내보내기](export)에,
`GanttHistoryApi`는 [히스토리](history)에 정리돼 있어요.
...
| `exportToPng` | `(options?: GanttExportOptions) => Promise<Blob>` | 차트를 래스터화해서 PNG blob으로 resolve해요. 다운로드는 일으키지 않아요. [내보내기](export)를 보세요. |
| `undo` | `() => void` | 가장 최근에 커밋된 제스처를 되돌리고 `onTasksChange`를 발생시켜요. [히스토리](history)를 보세요. |
| `redo` | `() => void` | 가장 최근에 취소된 제스처를 다시 실행하고 `onTasksChange`를 발생시켜요. [히스토리](history)를 보세요. |
