...
Behaviour and the `criticalPath` prop live in [Scheduling](../scheduling); running the core
without React is [Headless core](../headless-core).
...
|---|---|---|---|
| `calendar` | `WorkingCalendar` | `CALENDAR_DAYS` | The day unit every number on this page is counted in. `CALENDAR_DAYS` counts every day; a calendar from `createWorkingCalendar` counts only working days. See [`core-calendar.md`](core-calendar) |
...
[`core-scheduling.md`](core-scheduling).
...
These names appear above but are **not** in `src/index.ts`, so they cannot be imported from
...
`buildTaskGraph`, `linkKey` and `TaskGraph` are exported — see [`core-graph.md`](core-graph).
`CALENDAR_DAYS`, `createWorkingCalendar` and `WorkingCalendar` are exported too — see
[`core-calendar.md`](core-calendar).
...
`scheduleTasks` ([`core-scheduling.md`](core-scheduling)).
