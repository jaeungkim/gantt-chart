...
working-day calendar on changes in the chart is in [Scheduling](../scheduling).
...
[`scheduleTasks`](core-scheduling) and [`computeCriticalPath`](core-critical-path), so
...
Pass the calendar on as `options.calendar` to [`scheduleTasks`](core-scheduling) or
...
what a calendar does and does not affect is in [Scheduling](../scheduling).
