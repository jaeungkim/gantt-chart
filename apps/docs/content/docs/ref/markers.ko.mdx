...
마커와 밴드를 어디에 쓰는지, 타임라인을 스크롤할 때 어떻게 움직이는지는 [타임라인](../timeline)에 있어요.
...
각 값은 차트의 UTC `dayjs`로 그대로 넘어가요. 그래서 시간대가 없는 문자열(`'2025-06-01'`, `'2025-06-01T09:00'`)은 UTC 벽시계로 읽혀서 적힌 그대로의 위치에 놓여요. 작업 날짜와 같은 규칙이 적용돼요. [작업 데이터](../task-data)를 참고하세요.
...
- 렌더링된 범위는 작업들의 구간에 고정 버퍼를 더한 값이에요. `visibleStart`, `visibleEnd`, `infiniteScroll`이 이 범위를 넓힐 수 있어요. 마지막 작업보다 한참 뒤에 있는 마커는 그려지지 않아요. [타임라인](../timeline)을 참고하세요.
...
- [타임라인](../timeline) — 마커와 밴드가 언제 그려지는지, 렌더링된 범위가 어디까지인지.
- [테마](../theming) — `--gantt-marker`, `--gantt-marker-warning`, `--gantt-marker-label`, `--gantt-today-marker`, `--gantt-band-bg`를 비롯한 커스텀 프로퍼티 표 전체.
- [프롭](props) — `GanttProps` 전체 목록.
