...
two structural rules override every flag and are described in [Editing tasks](editing). A task
...
|---|---|---|
| `Ctrl`/`Cmd`+`Z`, `Ctrl`/`Cmd`+`Y`, `Ctrl`/`Cmd`+`Shift`+`Z` | the container `<section>` | Undo and redo. Ignored when `Alt` is held, or when the target is an `input`, `textarea`, `select` or contenteditable. See [Imperative API](imperative-api). |
...
keyboard. See [Events and cancellable changes](events).
