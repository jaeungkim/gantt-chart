...
the container's computed background color — see [Imperative API](imperative-api).

Next: [Headless core](headless-core) covers the scheduling, graph and calendar functions that
run without React.
