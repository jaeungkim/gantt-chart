...
- **A critical task.** With `criticalPath` on, `.gantt-task-bar.critical` sets `background` to
  ...
  covered in [Scheduling](scheduling).
...
The complete type shapes are in [Renderers](ref/renderers).
...
- **The two connector dots.** Dependency arrows are drawn by dragging from a bar's link handle, so
  ...
  render; see [Dependencies](dependencies).
...
- **`tabIndex` and `data-gantt-cell`.** The chart's focus manager finds a bar by its cell
  ...
  navigation stops reaching bars. See [Keyboard and screen readers](accessibility).
...
[Editing tasks](editing). For a milestone `endDate` equals `startDate`, so `durationMs` is `0`.
...
baselines are covered in [Scheduling](scheduling).
...
- **There are four render props, and no more.** There is no renderer for a task-list row, a group
  ...
  `columns[].render`, covered in [Task list and hierarchy](task-list).
...
Next: [Imperative API](imperative-api), which covers the `ref` handle — scrolling, zooming,
undo and PNG export.
