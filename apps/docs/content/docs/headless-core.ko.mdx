---
title: "헤드리스 코어"
description: "React와 DOM 없이 일정 계산하기"
---
...
`zustand`, `@tanstack/*`, 그리고 `components/`, `hooks/`, `stores/`, `constants/`,
`types/`, `utils/` 경로 그룹의 import를 막아요. 두 번째 규칙은 `window`, `document`,
...
[작업 그래프 헬퍼](ref/core-graph)에 있어요. 링크 타입 네 가지의 뜻은 [의존성](dependencies)에
있고요.
...
정책 자체는 [스케줄링](scheduling)에서 설명하고, 옵션과 반환 필드는 하나도 빠짐없이
[scheduleTasks](ref/core-scheduling)에 정리돼 있어요.
...
[스케줄링](scheduling)이 다뤄요. 필드별 타입은 [임계 경로](ref/core-critical-path)에 있어요.
...
[스케줄링](scheduling)에서 다뤄요. 옵션과 우선순위는 [근무일 달력](ref/core-calendar)에 있고요.
...
다음: [scheduleTasks](ref/core-scheduling). 스케줄링 진입점의 옵션과 반환 필드가 하나도 빠짐없이
정리돼 있어요.
