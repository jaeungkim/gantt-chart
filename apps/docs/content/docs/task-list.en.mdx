...
description: "The left pane, columns, and the `parentId` tree"
---
...
reach it — see [Locale and date formats](i18n) for what `locale` does change. To show dates any
...
to 120, and an optional `render`. The full type is in [GanttColumn](ref/columns).
...
are present only while `criticalPath` is on, covered in [Scheduling](scheduling).
...
by the intersection of every member's own drag window; see [Editing tasks](editing). The
scheduler also pins summary rows while `hierarchy` is on — see [Scheduling](scheduling).
...
props cover the timeline side only, in [Custom rendering](custom-rendering). Rows are
...
collapsed — see [Reordering rows](reordering).
