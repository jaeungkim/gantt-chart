...
const FEATURES = [
  {
    ...
    body: 'Drag to move, pull an edge to resize, drag a handle to set progress, draw an arrow to link two rows. Every gesture is cancellable before it commits.',
  },
  {
    title: 'Real scheduling',
    body: 'Auto-scheduling policies, a working-day calendar, critical path with per-task slack, and baselines to show the plan against the plan you had.',
  },
  {
    title: 'Headless core',
    body: 'scheduleTasks, computeCriticalPath and the tree helpers import no React and touch no DOM, so a server or a worker can run them.',
  },
  {
    title: 'Virtualized',
    ...
    <main className="flex flex-1 flex-col">
      <section className="mx-auto w-full max-w-6xl px-4 pt-16 pb-10 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-balance text-lg text-fd-muted-foreground">
          Virtualized rows, four dependency types, auto-scheduling and a critical path —
          with a scheduling core that runs without a DOM.
        </p>
      ...
      <section id="playground" className="mx-auto w-full max-w-6xl scroll-mt-20 px-4 pb-16">
