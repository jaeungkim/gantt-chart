...
function GanttChart({
  ...
  const visibleTasks = useMemo(() => {
    ...
    return getVisibleTasks(scheduledTasks, collapsedSet, tree);
  }, [hierarchy, collapsedSet, scheduledTasks, tree]);
  ...
  // Drawing a task on empty row space - one id per row, and a group header row owns no
  // task, so a range drawn on it belongs to no row
  ...
  // A keyboard edit is a gesture like a drag, so it commits the same way and is one
  // undo step - commitTasks, never setRawTasks
  const commitKeyboardEdit = useCallback(
    (updated: Task[]) => {
