...
/** Row ids, so a whole row model can be asserted in one line */
...
describe('packLanes', () => {
  ...
  it('treats a milestone as its single start point', () => {
    const rows = packLanes([
      task('m', '2025-01-01', '2025-12-31', { type: 'milestone' }),
      task('after', '2025-01-02', '2025-01-03'),
    ]);

    expect(idsOf(rows)).toEqual([['m', 'after']]);
  });
});
