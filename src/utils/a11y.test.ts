...
import type { Task, TaskTransformed } from 'types/task';
import {
  deleteTask,
  formatTaskAriaLabel,
  GanttKeyboardRow,
...
describe('formatTaskAriaLabel', () => {
  ...
  it('says milestone and gives its single date', () => {
    expect(
      formatTaskAriaLabel(
        {
          name: 'Launch',
...
describe('nudgeTaskDates', () => {
  ...
  it('never resizes a milestone or a summary row', () => {
    ...
    expect(nudgeTaskDates(raw(), milestone, 'right', 1, 'month')).toBeNull();
    expect(nudgeTaskDates(raw(), summary, 'left', 1, 'month')).toBeNull();
    // Moving either is still fine
    expect(nudgeTaskDates(raw(), milestone, 'bar', 1, 'month')).not.toBeNull();
  });
