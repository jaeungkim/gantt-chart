...
import { GanttDragBounds, GanttDragMode, GanttScaleKey } from "types/gantt";
import {
...
export function resolveKeyboardAction(
  ...
  // Editing first - a modifier turns the horizontal arrows into an edit, so it
  // must be checked before they are read as navigation
...
export function formatTaskAriaLabel(
  task: Pick<
    TaskTransformed,
    "name" | "startDate" | "endDate" | "type" | "isSummary"
  >,
  ...
  if (isMilestoneTask(task)) {
    parts.push("milestone", format(dayjs(task.startDate)));
  } else {
...
export function nudgeTaskDates(
  ...
  // The dates on screen, which for a summary are the ones rolled up from its children
